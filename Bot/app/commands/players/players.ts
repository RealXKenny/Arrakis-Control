...
module.exports = {
  ...
  async execute(
    ...
    try {
      ...
      const [onlineResponse, offlineResponse] =
        await Promise.all([
      ...
      const online = formatPlayers(
        onlineResponse,
        "online",
      );

      const offline = formatPlayers(
        offlineResponse,
        "offline",
      );
      ...
      const onlineCount = getPlayerCount(
        onlineResponse,
        online,
      );

      const offlineCount = getPlayerCount(
        offlineResponse,
        offline,
      );
          ...
          .addMediaGalleryComponents(
            new MediaGalleryBuilder().addItems(
              new MediaGalleryItemBuilder().setURL(
                "attachment://dune-server-players.png",
              ),
            ),
          )
      ...
      logger.error(
        "Unable to retrieve Dune player lists.",
        error,
      );
    }
  },
