...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("backups")
    .setDescription(
      "Show available Dune server backups and auto-backup status.",
    ),
  ...
  async execute(interaction: any): Promise<void> {
    ...
    try {
      ...
      const accentColor =
        DUNE_COLORS[
          Math.floor(Math.random() * DUNE_COLORS.length)
        ];
      ...
      const card = new ContainerBuilder()
        ...
        .addMediaGalleryComponents(
          new MediaGalleryBuilder().addItems(
            new MediaGalleryItemBuilder().setURL(
              "attachment://dune-server-backups.png",
            ),
          ),
        )

        .addTextDisplayComponents((text) =>
          text.setContent("## 🏜️ Dune Server Backups"),
        )
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            ["### 📦 Database Backups", backups.content].join("\n"),
          ),
        )
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            [
              "### 🕒 Schedule",
              `**Backup time:** \`${
                autoBackup.backupTime || "Unknown"
              } UTC\``,
              `**Interval:** \`${
                autoBackup.intervalHours ?? "Unknown"
              } hours\``,
              `**Retention:** \`${
                autoBackup.retentionDays ?? "Unknown"
              } days\``,
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            [
              "### ⚙️ Systemd Timer",
              `**Status:** \`${
                autoBackup.timerStatus || "Unknown"
              }\``,
              `**Unit:** \`${
                autoBackup.timerUnit || "Unknown"
              }\``,
              `**Activates:** \`${
                autoBackup.serviceUnit || "Unknown"
              }\``,
            ].join("\n"),
          ),
        )
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            `-# ${backups.count} backup${
              backups.count === 1 ? "" : "s"
            } available • Spice flows through Arrakis • Requested by ${
...
function parseBackups(response: unknown): BackupDisplay {
  ...
  const entries = Object.entries(objectResponse).filter(
    ([, value]) =>
      value !== null &&
      value !== undefined,
  );
...
function formatBackup(
  ...
  const timestampText =
    discordTimestampUnknown(timestamp);
...
function parseAutoBackupOutput(
  stdout: string,
): AutoBackupDisplay {
  const text = String(stdout).replace(/\r/g, "");

  const enabled = extractBoolean(
    text,
  ...
  const backupTime =
    ...
    extractValue(
      text,
      /backup[_\s-]?time:\s*([^\s]+)/i,
    );

  const intervalMatch = text.match(
    /Interval hours:\s*(\d+(?:\.\d+)?)/i,
  );

  const retentionMatch = text.match(
    /Retention:\s*(\d+)\s*days?/i,
  );

  const directory = extractValue(
    text,
    /Backup directory:\s*(.+?)(?=\s+Systemd timer:|\n|$)/i,
  );
  ...
  const timerSection =
    extractTimerSection(text);

  const timer =
    parseSystemdTimer(timerSection);
...
function parseAutoBackupObject(
  response: ApiObject,
): AutoBackupDisplay {
  const enabled = getBoolean(
    getFirstDefined(response, [
      "enabled",
      "active",
      "running",
      "autoBackup",
  ...
  const backupTime = toStringOrNull(
    getFirstDefined(response, [
      "backupTime",
      "backup_time",
      "time",
  ...
  const calculatedNext =
    ...
    calculateNextBackup({
      ...
      intervalHours,
      lastBackupTimestamp:
        getTimestampFromUnknown(lastRaw),
    });
...
function parseSystemdTimer(
  ...
  const timerLine = lines.find((line) =>
    /^(Sat|Sun|Mon|Tue|Wed|Thu|Fri)\s/i.test(
      line,
    ),
  );
...
function extractTimerSection(
  text: string,
): string {
  const index = text.search(
    /NEXT\s+LEFT\s+LAST\s+PASSED/i,
  );
...
function calculateNextBackup({
  ...
  backupTime: string | null;
  intervalHours: number | null;
  lastBackupTimestamp: TimestampValue;
}): string | null {
  const now = Date.now();

  if (lastBackupTimestamp) {
    const last =
      getTimestamp(lastBackupTimestamp);
...
function formatLastBackup(
  ...
  return formatLastBackupUnknown(
    timestamp,
  );
}

function formatLastBackupUnknown(
  timestamp: unknown,
): string {
  const discord =
    discordTimestampUnknown(timestamp);
  ...
  return `${discord} • ${relativeTimestampUnknown(
    timestamp,
  )}`;
}

function discordTimestampUnknown(
  value: unknown,
): string | null {
  const timestamp =
    getTimestampFromUnknown(value);
...
function relativeTimestampUnknown(
  value: unknown,
): string {
  const timestamp =
    getTimestampFromUnknown(value);
...
function getTimestampFromUnknown(
  value: unknown,
): number {
  if (
    typeof value === "string" ||
...
function detectTimerStatus(
  text: string,
): string {
  const match = text.match(
    /Systemd timer:\s*(enabled|disabled|active|inactive)/i,
  );
