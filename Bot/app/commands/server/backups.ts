  ...
  async execute(interaction: any): Promise<void> {
    ...
    try {
      ...
      const card = new ContainerBuilder()
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(`-# ${backups.count} backup${backups.count === 1 ? "" : "s"} available • Spice flows through Arrakis • Requested by ${interaction.user.tag}`),
        );
...
function parseAutoBackupOutput(stdout: string): AutoBackupDisplay {
  ...
  return {
    ...
    nextBackup: nextBackup ? (discordTimestampUnknown(nextBackup) ?? "Unknown") : "Unknown",
...
function parseAutoBackupObject(response: ApiObject): AutoBackupDisplay {
  ...
  const nextRaw = getFirstDefined(response, ["nextBackup", "next_backup", "next"]) ?? getFirstDefined(timerObject, ["next", "nextBackup"]);
  ...
  return {
    ...
    retentionDays,
    directory,
    timerStatus: toStringOrNull(getFirstDefined(response, ["timerStatus", "timer_status"])) ?? toStringOrNull(timerObject["status"]),

    timerUnit: toStringOrNull(getFirstDefined(response, ["timerUnit", "timer_unit"])) ?? toStringOrNull(timerObject["unit"]),

    serviceUnit: toStringOrNull(getFirstDefined(response, ["serviceUnit", "service_unit"])) ?? toStringOrNull(timerObject["service"]),

    nextBackup: calculatedNext ? (discordTimestampUnknown(calculatedNext) ?? "Unknown") : "Unknown",

    lastBackup: lastRaw ? formatLastBackupUnknown(lastRaw) : "Unknown",
  };
}
...
function findLastBackup(text: string): ParsedBackup | null {
  ...
  const backups: ParsedBackup[] = matches.map((match) => ({
...
function formatLastBackupUnknown(timestamp: unknown): string {
...
function getTimestampFromUnknown(value: unknown): number {
  ...
  if (isObject(value)) {
    const timestamp = getFirstDefined(value, ["timestamp", "createdAt", "created_at", "date"]);

    if (typeof timestamp === "string" || typeof timestamp === "number" || timestamp instanceof Date || timestamp === null || timestamp === undefined) {
...
function getFirstDefined(object: ApiObject, keys: string[]): unknown {
...
function createBackupBanner({ serverName, count }: { serverName: string; count: number; autoBackup: boolean }) {
