...
function formatServer(
  ...
  const location = getStringOrNull(
    server.location,
    server.datacenter,
    server.data_center,
    server.region,
  ...
  async execute(interaction: any) {
    ...
    try {
      const response: unknown =
        await client.convoyApi.request(
          "GET",
          "/api/client/servers",
        );
      ...
      if (servers.length > 25) {
        ...
        card.addTextDisplayComponents(
          (text) =>
            text.setContent(
              `-# Showing 25 of ${servers.length} servers.`,
            ),
        );
      }
      ...
      card.addTextDisplayComponents(
        (text) =>
          text.setContent(
            "-# Spice flows through Arrakis • Convoy Control",
          ),
      );
        ...
        ...createV2Response(
          ...
          [
            createDuneBanner({
              filename: IMAGE_NAME,
              title: "Advin VPS",
              subtitle: `${servers.length} SERVER${
                servers.length === 1
                  ? ""
                  : "S"
              }`,
              detail:
                "CONVOY CONTROL PANEL",
            }),
          ],
        ),
        allowedMentions: {
      ...
      logger.error(
        `Unable to retrieve Advin VPS servers. ${errorDetails.message}`,
        error,
      );
