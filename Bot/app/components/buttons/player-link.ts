...
module.exports = {
  ...
  async execute(interaction: ButtonInteraction): Promise<void> {
    ...
    const linked = await interaction.client.discordAdapter.getCurrentPlayer(createActorContext(interaction, "player-link"));
    ...
    if (linked?.linked === true) {
      ...
      const card = new ContainerBuilder()
        ...
        .addActionRowComponents((row) => row.setComponents(new ButtonBuilder().setCustomId("player-unlink").setLabel("Unlink Account").setStyle(ButtonStyle.Danger)));
