...
import {
  createLogger,
} from "../../../infrastructure/core/logger";

const logger = createLogger("PLAYER LINK");

const LINKED_ROLE_ID =
  process.env.LINKED_PLAYER_ROLE_ID;
...
module.exports = {
  ...
  async execute(
    ...
    const result =
      (await interaction.client.discordAdapter.verifyPlayerLink(
        createActorContext(
          interaction,
          "player-verify",
        ),
        code,
      )) as VerifyPlayerLinkResult;

    if (result?.ok) {
      await applyLinkedMemberProfile(
        interaction,
        result,
      );
    }
    ...
    if (result?.ok) {
      await interaction.client.auditLogger?.playerLinked(
        interaction,
        result,
      );
    }
...
async function applyLinkedMemberProfile(
  interaction: ModalSubmitInteraction,
  result: VerifyPlayerLinkResult,
): Promise<void> {
  ...
  try {
    ...
    logger.info(
      `Applied linked role and nickname for ${interaction.user.tag}.`,
    );
  } catch (error: unknown) {
    logger.warn(
