...
module.exports = {
  ...
  async execute(member: GuildMember): Promise<void> {
    await member.client.auditLogger?.sendTo(member.client.auditLogger.activityChannelId, "Member left", [`**User:** ${member.user.tag} (${member.id})`, `**Guild:** ${member.guild.name}`]);
    ...
    const container = createContainer({
      ...
      color: 0xc58b45,
    }).addMediaGalleryComponents(new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL("attachment://member-goodbye.png")));
