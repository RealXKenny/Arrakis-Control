...
const version_1 = require("../../../infrastructure/config/version");
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const formatUptime = (inputSeconds) => {
        ...
        const formatMemory = (bytes) => `${(bytes /
            1024 /
            1024).toFixed(1)} MB`;
        ...
        while (ctx.measureText(displayServerName).width >
            maxServerNameWidth &&
        ...
        const infoCard = new discord_js_1.ContainerBuilder()
            ...
            .addMediaGalleryComponents(new discord_js_1.MediaGalleryBuilder().addItems(new discord_js_1.MediaGalleryItemBuilder().setURL("attachment://dune-server-info.png")))
            ...
            "### 🏜️ Bot",
            `**Version:** v${(0, version_1.getBotVersion)()}`,
            `**User ID:** \`${client.user.id}\``,
            `**Created:** <t:${Math.floor(client.user
                .createdTimestamp /
                1000)}:D>`,
        ].join("\n")))
            .addSeparatorComponents((separator) => separator.setSpacing(discord_js_1.SeparatorSpacingSize.Small))
            ...
            `**Online since:** <t:${Math.floor((Date.now() -
                client.uptime) /
                1000)}:R>`,
        ].join("\n")))
            .addSeparatorComponents((separator) => separator.setSpacing(discord_js_1.SeparatorSpacingSize.Small))
        ...
        await interaction.reply((0, componentFactory_1.createV2Response)([infoCard], [banner]));
    },
};
