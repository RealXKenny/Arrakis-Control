...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const sent = await interaction.reply({
            components: [
                new discord_js_1.ContainerBuilder()
        ...
        ctx.fillText(`WebSocket: ${websocketPing >= 0
            ? `${websocketPing}ms`
            : "Measuring..."}`, 180, 305);
        ctx.font =
            "18px sans-serif";
        ...
        const pingCard = new discord_js_1.ContainerBuilder()
            .setAccentColor(accentColor)
            .addMediaGalleryComponents(new discord_js_1.MediaGalleryBuilder().addItems(new discord_js_1.MediaGalleryItemBuilder().setURL("attachment://dune-server-ping.png")))
            .addTextDisplayComponents((text) => text.setContent("## 🏓 Pong!"))
        ...
        await interaction.editReply({
            ...(0, componentFactory_1.createV2Response)([pingCard], [banner]),
            allowedMentions: {
                parse: [],
            },
        });
