...
const players_1 = require("../../../modules/formatters/players");
...
const logger = (0, logger_1.createLogger)("PLAYERS");
const PAGE_SIZE = 20;
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const online = (0, players_1.formatPlayers)(onlineResponse, "online");
            const offline = (0, players_1.formatPlayers)(offlineResponse, "offline");
             ...
             * --------------------------------------------------
             */
            const canvas = (0, canvas_1.createCanvas)(1200, 300);
            ...
            const drawDune = (y, height, color, offset = 0) => {
                ...
                for (let x = 0; x <= canvas.width; x += 15) {
                    const wave = Math.sin((x + offset) / 120) *
                        height *
                        0.25 +
                        Math.sin((x + offset) / 240) *
                            height *
                            0.18 +
                        Math.sin((x + offset) / 70) *
                            height *
                            0.05;
                    ctx.lineTo(x, y + wave);
                }
            ...
            while (ctx.measureText(displayServerName).width > maxServerNameWidth &&
                displayServerName.length > 3) {
                displayServerName =
                    displayServerName.slice(0, -4) +
                        "...";
            }
             ...
             * --------------------------------------------------
             */
            const playersCard = new discord_js_1.ContainerBuilder()
                .setAccentColor(0xc58b45)
                .addMediaGalleryComponents(new discord_js_1.MediaGalleryBuilder().addItems(new discord_js_1.MediaGalleryItemBuilder().setURL("attachment://dune-server-players.png")))
            ...
            await interaction.editReply({
                ...(0, componentFactory_1.createV2Response)([playersCard], [banner]),
                allowedMentions: {
                    parse: [],
                },
            });
            ...
            const errorCard = new discord_js_1.ContainerBuilder()
                ...
                .addSeparatorComponents((separator) => separator.setSpacing(discord_js_1.SeparatorSpacingSize.Small))
                .addTextDisplayComponents((text) => text.setContent([
                "### ⚠️ Unable to retrieve players",
                "The Dune server player list could not be retrieved. Please try again later.",
            ].join("\n")));
            await interaction.editReply({
