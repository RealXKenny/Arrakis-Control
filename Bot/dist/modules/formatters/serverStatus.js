...
exports.formatServerStatus = formatServerStatus;
function formatServerStatus(status, performance = null, readiness = null, ports = null, services = null) {
    ...
    return {
        healthy: overview.Overall?.toUpperCase() === "READY" &&
            status?.exitCode === 0,
        overview: [
...
function parseTable(section) {
    ...
    return lines
        .slice(1)
        .map((line) => columnStarts.map((start, index) => line
...
function formatServices(response) {
    ...
    const lines = output
        ...
        .map((line) => line.trimEnd())
        .filter(Boolean);
    const serviceLines = lines.filter((line) => !line.startsWith("NAMES") &&
        /^(redblink-|dune-)\S+\s+/.test(line));
