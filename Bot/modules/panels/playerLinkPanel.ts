    ...
    .addMediaGalleryComponents(
      new MediaGalleryBuilder().addItems(
        new MediaGalleryItemBuilder()
          ...
          .setDescription(
            "Dune desert landscape for character linking",
          ),
      ),
    )
...
async function ensurePlayerLinkPanel(
  client: Client,
  ...
  if (!client.user) {
    throw new Error(
      "Cannot create player link panel before the Discord client is ready.",
    );
  }
  ...
  if (!channel || !channel.isSendable()) {
    throw new Error(
      `Panel channel ${channelId} is not a sendable channel.`,
    );
  }
  ...
  if (existingPanel) {
    ...
    logger.info(
      `Updated the player link panel in channel ${channelId}.`,
    );
  ...
  logger.info(
    `Posted player link panel in channel ${channelId}.`,
  );
}
...
function createPlayerLinkBanner(): AttachmentBuilder {
  ...
  context.fillText(
    "Your path through Arrakis begins here.",
    67,
    232,
  );
