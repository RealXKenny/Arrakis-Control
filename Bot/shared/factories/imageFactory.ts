...
function drawDuneBanner(
  ...
  height: number,
  {
    ...
    subtitle,
    detail,
  }: Omit<DuneBannerOptions, "filename">,
): void {
...
function createDuneBanner({
  filename,
  title,
  subtitle,
  detail,
}: DuneBannerOptions): AttachmentBuilder {
...
async function createMemberBanner({
  filename,
  title,
  member,
}: MemberBannerOptions): Promise<AttachmentBuilder> {
  const canvas = createCanvas(1200, 400);
  const context = canvas.getContext("2d");
