...
function BaseCard({
  ...
  const powerPercent = clampPercent(
    (runtimeSeconds / MAX_POWER_SECONDS) * 100
  );
        ...
        bases.map(async (base) => {
          ...
          try {
            if (
              ...
              if (
                waterResult.value.ok
              ) {
                water =
                  await waterResult.value.json();
              } else {
                waterError =
                  `Water API returned ${waterResult.value.status}`;
              }
            } else {
              waterError =
                waterResult.reason?.message ||
                'Water request failed';
            }
          } catch (error) {
            waterError =
              error?.message ||
              'Invalid water response';
          }
          ...
          try {
            if (
              ...
              if (
                inventoryResult.value.ok
              ) {
                inventory =
                  await inventoryResult.value.json();
              } else {
                inventoryError =
                  `Inventory API returned ${inventoryResult.value.status}`;
              }
            } else {
              inventoryError =
