    ...
    try {
      ...
      const drawDune = (y, height, color, offset = 0) => {
        ...
        for (let x = 0; x <= canvas.width; x += 15) {
          const wave =
            Math.sin((x + offset) / 120) * height * 0.25 +
            Math.sin((x + offset) / 240) * height * 0.18 +
            Math.sin((x + offset) / 70) * height * 0.05;
      ...
      const playersCard = new ContainerBuilder()
        ...
        .addMediaGalleryComponents(
          new MediaGalleryBuilder().addItems(
            new MediaGalleryItemBuilder().setURL('attachment://dune-server-players.png'),
          ),
        )
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            [
              `### 🟢 ${online.heading}`,
              online.content,
              online.truncated ? `_${online.truncated}_` : null,
            ]
              .filter(Boolean)
              .join('\n'),
          ),
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            [
              `### 🔴 ${offline.heading}`,
              offline.content,
              offline.truncated ? `_${offline.truncated}_` : null,
            ]
              .filter(Boolean)
              .join('\n'),
          ),
