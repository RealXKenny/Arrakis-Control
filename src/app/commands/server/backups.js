...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const accentColor =
        DUNE_COLORS[Math.floor(Math.random() * DUNE_COLORS.length)];
  ...
  if (response && typeof response === "object") {
    ...
    if (entries.length) {
      return {
        ...
        content: entries
          .map(
            ([key, value]) => `**${formatLabel(key)}:** ${formatValue(value)}`,
          )
          .join("\n"),
      };
    }
...
function formatBackup(backup, index) {
  ...
  const timestamp =
    backup.timestamp ?? backup.createdAt ?? backup.created_at ?? backup.date;
...
function parseAutoBackupOutput(stdout) {
  ...
  const timerStatus =
    extractValue(text, /Systemd timer:\s*([^\s]+)/i) ?? detectTimerStatus(text);
...
function parseAutoBackupObject(response) {
  ...
  const backupTime =
    response.backupTime ?? response.backup_time ?? response.time ?? null;
  ...
  const lastRaw =
    response.lastBackup ?? response.last_backup ?? response.last ?? null;
  ...
  return {
    ...
    directory,
    timerStatus:
      response.timerStatus ?? response.timer_status ?? timer.status ?? null,
    timerUnit: response.timerUnit ?? response.timer_unit ?? timer.unit ?? null,
    serviceUnit:
      response.serviceUnit ?? response.service_unit ?? timer.service ?? null,
...
function parseSystemdTimer(text) {
  ...
  const timerLine = lines.find((line) =>
    /^Sat|^Sun|^Mon|^Tue|^Wed|^Thu|^Fri/i.test(line),
  );
...
function getBoolean(value) {
  ...
  if (typeof value === "string") {
    ...
    if (
      ["true", "yes", "enabled", "active", "running", "on"].includes(normalized)
    ) {
      return true;
    }

    if (
      ["false", "no", "disabled", "inactive", "stopped", "off"].includes(
        normalized,
      )
    ) {
      return false;
...
function createBackupBanner({ serverName, username, count, autoBackup }) {
  ...
  ctx.fillText(
    autoBackup ? "AUTO-BACKUPS ENABLED" : "AUTO-BACKUPS DISABLED",
    600,
    298,
  );
