...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      const response = await interaction.client.convoyApi.request(
        "GET",
        "/api/client/servers",
      );
      ...
      const card = new ContainerBuilder()
        ...
        .addTextDisplayComponents((text) =>
          text.setContent("## Advin VPS Servers"),
        )
        .addTextDisplayComponents((text) =>
      ...
      await interaction.editReply(
        `Unable to retrieve Advin VPS servers: ${error.message}`,
      );
    }
  },
};
...
function formatServer(server) {
  const name =
    ...
    "Unnamed server";
  const state =
    server.state ?? server.power_state ?? server.status ?? "Unknown";
