...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      const [status, performance, readiness, ports, services] =
        await Promise.all([
      ...
      const healthy =
        statusData.overall === "READY" && readinessData.failed === 0;
      ...
      const statusCard = new ContainerBuilder()
        ...
        .addTextDisplayComponents((text) =>
          text.setContent("## 🏜️ Dune Server Status"),
        )
            ...
            [
              ...
              statusData.gameServers.length
                ? statusData.gameServers
                    .map(
                      (server) =>
                        `${server.state === "READY" ? "🟢" : "🔴"} **${server.map}** — \`${server.state}\` — ${server.uptime}`,
                    )
                    .join("\n")
            ...
            [
              ...
              servicesData.containers.length
                ? servicesData.containers
                    .map(
                      (service) =>
                        `${service.status.includes("healthy") ? "🟢" : "🟡"} \`${service.name}\` — ${service.status}`,
                    )
                    .join("\n")
            ...
            [
              ...
              readinessData.listeners.length
                ? readinessData.listeners
                    .map(
                      (listener) =>
                        `${listener.ok ? "🟢" : "🔴"} **${listener.name}** — \`${listener.port}\` — ${listener.status}`,
                    )
                    .join("\n")
            ...
            [
              ...
              portsData.length
                ? portsData
                    .map(
                      (port) =>
                        `${port.ok ? "🟢" : "🔴"} **${port.name}** — \`${port.address}\` — ${port.status}`,
                    )
                    .join("\n")
            ...
            [
              ...
              servicesData.services.length
                ? servicesData.services
                    .map(
                      (service) =>
                        `${getServiceIndicator(service.status)} \`${service.name}\` — ${service.status}`,
                    )
                    .join("\n")
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            [
              "### 🗄️ Database",
              `**World partitions:** ${statusData.worldPartitions ?? "Unknown"}`,
            ].join("\n"),
          ),
        )
        .addSeparatorComponents((separator) =>
        ...
        .addTextDisplayComponents((text) =>
          text.setContent(
            [
              "### 🤖 Automation",
              `**Autoscaler:** ${statusData.autoscaler || "UNKNOWN"}`,
              `**Auto updates:** ${statusData.autoUpdates || "UNKNOWN"}`,
            ].join("\n"),
          ),
        )
        .addSeparatorComponents((separator) =>
...
function parseServerStatus(stdout) {
  ...
  if (funcomSection) {
    ...
    const populationDeclaration = section.match(
...
function createStatusBanner({
  ...
  glow.addColorStop(
    0,
    healthy ? "rgba(255, 190, 90, 0.55)" : "rgba(180, 55, 35, 0.55)",
  );

  glow.addColorStop(
    0.45,
    healthy ? "rgba(190, 100, 40, 0.22)" : "rgba(130, 35, 25, 0.25)",
  );
  ...
  ctx.fillText(
    healthy ? "● SERVER OPERATIONAL" : "● SERVER ATTENTION",
    64,
    205,
  );
  ...
  ctx.fillText(
    `CPU ${formatPercent(cpuPercent)}  •  Memory ${formatPercent(memoryPercent)}  •  Disk ${formatPercent(diskPercent)}`,
    64,
    345,
  );
  ...
  drawFittedText(
    ctx,
    `Uptime ${uptime || "Unknown"} • Spice flows through Arrakis`,
    64,
    375,
    850,
    "17px sans-serif",
