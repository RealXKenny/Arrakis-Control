...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const linked =
      await interaction.client.discordAdapter.getCurrentPlayer(actor);
    if (!linked?.linked) {
    ...
    const playerList = await interaction.client.duneApi.call(
      "GET",
      "/api/players",
      {
        query: {
    ...
    if (
      String(linked.onlineStatus).toLowerCase() === "online" ||
      isOnline(profile)
    ) {
      await interaction.editReply(
    ...
    const result = await interaction.client.duneApi.importBlueprint(
      playerId,
      files.first(),
    );
    await interaction.editReply(
...
function findLinkedPlayer(response, linked) {
  ...
  return (
    rows.find(
      (player) =>
        String(
          player.player_controller_id ?? player.playerControllerId ?? "",
        ) === controllerId,
    ) ??
    rows.find(
      (player) =>
