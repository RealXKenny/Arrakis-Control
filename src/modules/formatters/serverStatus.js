function formatServerStatus(
  ...
  return {
    healthy:
      overview.Overall?.toUpperCase() === "READY" && status?.exitCode === 0,
...
function parseTable(section) {
  ...
  const columnStarts = [...lines[0].matchAll(/\S+/g)].map(
    (match) => match.index,
  );
  ...
  return lines
    ...
    .map((line) =>
      columnStarts.map((start, index) =>
        line.slice(start, columnStarts[index + 1]).trim(),
      ),
    )
...
function formatServices(response) {
  ...
  const serviceLines = lines.filter(
    (line) =>
      !line.startsWith("NAMES") && /^(redblink-|dune-)\S+\s+/.test(line),
  );
