...
if (!changelogRelease) throw new Error(`CHANGELOG.json does not contain a release entry for version ${version}.`);
...
function buildNotes({ version: releaseVersion, changelogRelease }) {
  ...
  for (const change of changes) groups.get(change.type ? categoryFor(change.type) : categoryFor(change.subject)).push(change);

  const lines = [`# Arrakis Control v${releaseVersion}`, "", changelogRelease?.summary ?? null, changelogRelease?.date ? `Release date: ${changelogRelease.date}` : null, "", "## Docker image", "", "```bash", `docker pull realxkenny/arrakis-control:${releaseVersion}`, "```", "", "## Changes", ""].filter((line) => line !== null);
...
function categoryFor(subject) {
  ...
  return { feat: "Features", feature: "Features", fix: "Fixes", perf: "Performance", docs: "Documentation", refactor: "Refactoring", chore: "Maintenance", maintenance: "Maintenance", build: "Maintenance", ci: "Maintenance", test: "Maintenance" }[type] ?? "Other changes";
}
