...
if (!changelogRelease)
  throw new Error(
    `CHANGELOG.json does not contain a release entry for version ${version}.`,
  );
...
function buildNotes({ version: releaseVersion, changelogRelease }) {
  ...
  for (const change of changes)
    groups
      .get(change.type ? categoryFor(change.type) : categoryFor(change.subject))
      .push(change);
  ...
  if (changes.length === 0)
    lines.push("No changes were recorded for this release.");
...
function loadChangelogRelease(releaseVersion) {
  ...
  try {
    ...
    return (
      changelog.releases?.find(
        (release) => release.version === releaseVersion,
      ) ?? null
    );
  } catch (error) {
    console.warn(
      `Unable to read CHANGELOG.json; using commit history instead: ${error.message}`,
    );
    return null;
  }
}
...
function categoryFor(subject) {
  const type =
    subject.match(/^(\w+)(?:\(.+?\))?!?:/i)?.[1]?.toLowerCase() ??
    subject.toLowerCase();
  return (
    {
      feat: "Features",
